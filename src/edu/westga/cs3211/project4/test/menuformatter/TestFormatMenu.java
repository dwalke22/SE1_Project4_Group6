package edu.westga.cs3211.project4.test.menuformatter;

import static org.junit.jupiter.api.Assertions.*;

import java.util.ArrayList;

import org.junit.jupiter.api.Test;

import edu.westga.cs3211.project4.formatter.MenuFormatter;
import edu.westga.cs3211.project4.model.Menu;
import edu.westga.cs3211.project4.model.MenuItem;

class TestFormatMenu {

	@Test
	void testThreeItems() {
		MenuItem item1 = new MenuItem("test1",5.70);
		MenuItem item2 = new MenuItem("test2",7.45);
		MenuItem item3 = new MenuItem("test3",10.0);
		ArrayList<MenuItem> arrayList = new ArrayList<MenuItem>();
		arrayList.add(item1);
		arrayList.add(item2);
		arrayList.add(item3);
		
		Menu testMenu = new Menu(arrayList,"restaurant");
		
		String correctFormatting = "restaurant" + System.lineSeparator() +
				"test1 - $5.7" +  System.lineSeparator() +
				"test2 - $7.45" +  System.lineSeparator() +
				"test3 - $10.0" +  System.lineSeparator();
		
		MenuFormatter testFormatter = new MenuFormatter();
		assertEquals(testFormatter.formatMenu(testMenu),correctFormatting);
	}
	
	@Test
	void testOneItem() {
		MenuItem item1 = new MenuItem("test1",5.70);
		ArrayList<MenuItem> arrayList = new ArrayList<MenuItem>();
		arrayList.add(item1);

		
		Menu testMenu = new Menu(arrayList,"restaurant");
		
		String correctFormatting = "restaurant" + System.lineSeparator() +
				"test1 - $5.7" +  System.lineSeparator();
		
		MenuFormatter testFormatter = new MenuFormatter();
		assertEquals(testFormatter.formatMenu(testMenu),correctFormatting);
	}
	
	@Test
	void testNullItems() {
		
		MenuFormatter testFormatter = new MenuFormatter();
		assertThrows(IllegalArgumentException.class,
				()-> {
					testFormatter.formatMenu(null);
				});
	}
	
	

}
